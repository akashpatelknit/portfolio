				...
				<div>
					<div className=" flex flex-col md:flex-row lg:flex-row items-center  justify-between gap-4 py-10">
						<div className=" flex-1 max-w-2xl ">
							...
							{/* // projectdescription */}
							<p className="text-[1.5rem] md:text-[2rem] font-normal leading-[110%] py-5">
							...
							<div className=" flex flex-wrap gap-3 py-5">
								{data[0]?.node.stacks.map((tech) => (
									<span
										className=" bg-gray-200 p-1 px-2 rounded-md w-fit"
										key={tech.title}
									>
										{tech.title}
									</span>
								...
								</p>
								{
									<div className=" flex flex-row gap-3">
										{data[0]?.node.sectors.map((role) => (
											<span
												key={role.title}
												className=" bg-gray-200 w-fit px-2 py-1 rounded-md"
											>
												{role.title}
											</span>
										))}
							...
							<div className="py-3">
								...
								{
									<div className=" flex flex-wrap gap-3">
										{data[0]?.node.developerRoles.map(
											(role) => (
												<span
													key={role.title}
													className=" bg-gray-200 w-fit px-2 py-1 rounded-md"
							...
							<div className=" py-3 flex gap-3">
								{data[0].node?.livelink && (
									<div>
										<Link
											href={data[0].node?.livelink}
											className=" "
										>
											Live Link
										</Link>
								...
								{data[0].node?.githublink && (
									<div>
										<Link
											href={data[0].node?.githublink}
											className=" "
										>
											Github Link
										</Link>
